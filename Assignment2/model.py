...
class MLP():
    ...
    def MiniBatch(self,data,GDparams,experiment,verbose,save):
        ...
        if save:
            self.save(hist,GDparams,experiment)

        return hist

    def cyclicLearning(self,data, GDparams, experiment, verbose, save, freq=10):
    ...
    def save(self, hist, GDparams, experiment):
        eta = GDparams['eta']
        n_batch = GDparams['n_batch']
        epochs = GDparams['epochs']
...
class Search():
    ...
    def __init__(self, l_min, l_max,n_lambda, p1,params1,p2,params2,epoch=False):
        self.l_min = l_min
        self.l_max = l_max
    ...
    def grid_search(self, data, GDparams, lmda):
        '''remember, if the cycle length or ns should be changed, 
        this is done via epoch, that means epoch needs to be changed beforehand'''
